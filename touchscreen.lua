digilines.update_ts_formspec = function (pos)
...
digilines.ts_on_receive_fields = function (pos, formname, fields, sender)
	local meta = minetest.get_meta(pos)
	...
	if not init then
		if fields.save then
			...
			meta:set_int("init",1)
			digilines.update_ts_formspec(pos)
		end
	else
...
digilines.process_command = function (meta, data, msg)
...
digilines.ts_on_digiline_receive = function (pos, node, channel, msg)
	local meta = minetest.get_meta(pos)
	local setchan = meta:get_string("channel")
	...
	local data = minetest.deserialize(meta:get_string("data")) or {}
	if msg.command then
		data = digilines.process_command(meta,data,msg)
	else
		for _,i in ipairs(msg) do
			if type(i) == "table" and i.command then
				data = digilines.process_command(meta,data,i) or data
			end
		end
	end
...
minetest.register_node("digilines:touchscreen", {
	description = "Digilines Touchscreen",
	groups = {cracky=3},
	on_construct = function(pos)
		digilines.update_ts_formspec(pos,true)
	end,
	drawtype = "nodebox",
	tiles = {
	...
	node_box = {
    	...
    	_digilines_channelcopier_fieldname = "channel",
	_digilines_channelcopier_onset = function(pos)
		minetest.get_meta(pos):set_int("init",1)
		digilines.update_ts_formspec(pos)
	end,
	on_receive_fields = digilines.ts_on_receive_fields,
	digiline = 
	{
		receptor = {},
		effector = {
			action = digilines.ts_on_digiline_receive
		},
	},
})

minetest.register_craft({
	output = "digilines:touchscreen",
	recipe = {
